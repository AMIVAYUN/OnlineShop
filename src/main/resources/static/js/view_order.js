...
function ListPrint(res){
    for(i=0; 0<res.length;i++){
        var html='<tr id="'+res[i].orderId+'"><td class="order_id">'+res[i].orderId+'</td><td class="username">'+res[i].user_name+'</td><td class="totalprice">'+res[i].totalprice+'</td><td class="orderdate">'+res[i].orderdate+'</td><td class="orderitem"><ul class="orderitem_list"></ul></td><td class="delivery_place">'+res[i].address+'</td><td class="delivery_stat">'+res[i].deliverystatus+'</td></tr>'
